...
git config --global alias.lp "log --color --pretty=format:'%C(magenta)%h%Creset → %C(italic)%s %Creset%Cgreen(%cr) %C(italic blue)<%an>%Creset %C(bold black yellow)%d%Creset' --abbrev-commit"
git config --global alias.lr "lp --reverse"
git config --global alias.l "lp --graph"
git config --global alias.lg "l --all"
...
git config --global alias.fp "!_fp() { git fetch --prune origin \$1:\$1; }; _fp"
git config --global alias.fpm "fp main"
