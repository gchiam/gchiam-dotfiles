...
git config --global alias.lgm "log --all --color --graph --pretty=format:'%C(magenta)%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr)%C(bold blue)<%an>%Creset' --abbrev-commit master..."
git config --global alias.g "grep --extended-regexp --break --heading --line-number"
